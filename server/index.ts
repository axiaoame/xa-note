...
app.put('/api/categories/:id', requireAuth, async c => {
  ...
  const oldCategory = db.prepare('SELECT name FROM categories WHERE id = ?').get(id) as any
...
// 确保上传目录存在
function ensureUploadDir(noteId: string) {
...
app.post('/api/trash/:id/restore', requireAuth, c => {
  ...
  // 获取回收站中的笔记
  const trashNote = db.prepare('SELECT * FROM trash WHERE id=?').get(id) as any
...
app.delete('/api/trash/:id', requireAuth, c => {
  ...
  // 获取笔记信息用于日志
  const trashNote = db.prepare('SELECT title FROM trash WHERE id=?').get(id) as any
...
app.post('/api/share/:id', requireAuth, async c => {
  ...
  // 记录创建分享日志
  try {
    const note = db.prepare('SELECT title FROM notes WHERE id=?').get(id) as any
    LogService.log({
      user_id: 'admin',
...
app.delete('/api/shares/:id', requireAuth, c => {
  ...
  // 获取分享信息用于日志
  const share = db.prepare('SELECT note_id FROM shares WHERE id=?').get(id) as any
...
async function testWebDAVConnection(webdav: any) {
...
async function backupNotesToWebDAV(webdav: any) {
  // 获取所有笔记和分类
  ...
  for (const note of notes) {
    try {
      ...
      if (tags.length > 0) {
        content += `**标签：** ${tags.map((tag: string) => `#${tag}`).join(' ')}\n\n`
      }
...
async function backupDatabaseToWebDAV(webdav: any) {
  // 使用新的目录命名规则
  const timestamp = new Date().toISOString().slice(0, 10).replace(/-/g, '') // YYYYMMDD
  const backupDir = `xanote-backup-${timestamp}`
...
async function uploadToWebDAV(webdav: any, filePath: string, content: string) {
  const url = webdav.url.endsWith('/') ? webdav.url : webdav.url + '/'
